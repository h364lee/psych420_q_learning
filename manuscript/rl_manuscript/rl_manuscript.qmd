...
phasically at reward delivery; after learning, they fired at the appearance of the cue that predicted reward, not at the reward itself; and when a predicted reward failed to arrive, their firing rate dropped below baseline at exactly the moment the reward would have occurred (Schultz et al., 1997). This suggested that dopaminergic neurons do not simply encode the occurrence of reward, but rather the descrepancy between the predicted and actual timing and magnitude of reward outcomes (Schultz et al., 1997). 
...
Building on this, Frank et al. (2007) asked whether naturally occurring individual differences in Go and NoGo learning in healthy participants could be similarly attributed to dopaminergic mechanisms. They fit a modified Q-learning model with separate learning rates for positive ($\alpha_G$​) and negative ($\alpha_L$​) prediction errors to behavioural data from 69 healthy participants, and showed that the recovered parameters were predicted by dopamine receptor gene variants — DARPP-32 predicted $\alpha_G$ and DRD2 predicted $\alpha_L$. This demonstrated that Q-learning learning rates recovered from behaviour reflect specific biological mechanisms, and that individual differences in reinforcement learning can be characterised computationally from behaviour alone.

### Our Simulation 
To demonstrate how Q-learning can be applied as a computational tool in psychiatry, we extended our Q-learning maze implementation (@sec-implementation)
the Q-learning implementation described in  was extended by 
replacing the single learning rate $\alpha$ with two separate parameters: $\alpha^+$ 
for positive prediction errors, corresponding to D1-mediated Go learning, and 
$\alpha^-$ for negative prediction errors, corresponding to D2-mediated NoGo learning. 
This modification follows the approach of Frank et al. (2007), who showed that 
separate learning rates for positive and negative outcomes reflect distinct 
dopaminergic receptor mechanisms. Four agent profiles were defined to represent 
distinct dopaminergic states (@tbl-agents), each trained for 500 episodes and 
averaged across 20 independent runs to reduce stochastic variability.
